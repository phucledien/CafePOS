...
class CreateOrderViewController: UIViewController {
    ...
    @IBOutlet weak var btnSelectDrink: CustomButton!
    @IBOutlet weak var btnSelectTable: CustomButton!
    ...
    private func bindUI() {
        ...
        viewModel.outputs.isSelectedTable.asDriver()
            .drive(onNext: { [weak self] isSelected in
                guard let strongSelf = self else {return}
                strongSelf.btnSelectTable.setTitle("Chọn Bàn", for: .normal)
                if isSelected {
                    strongSelf.btnSelectTable.setTitle("Thay Đổi Bàn", for: .normal)
                }
            })
            .disposed(by: disposeBag)
        
        viewModel.outputs.isSelectedDrink.asDriver()
            .drive(onNext: { [weak self] isSelected in
                guard let strongSelf = self else {return}
                strongSelf.btnSelectDrink.setTitle("Chọn Món", for: .normal)
                if isSelected {
                    strongSelf.btnSelectDrink.setTitle("Thay Đổi Món", for: .normal)
                }
            })
            .disposed(by: disposeBag)
        ...
        btnSubmit.rx.tap
            .subscribe(onNext: { [weak self] in
                SVProgressHUD.show()
                guard let strongSelf = self else {return}
                strongSelf.viewModel.outputs.createOrder()
                    .subscribe(onNext: { _ in
                        let tableListVC = strongSelf.navigationController?.viewControllers.first as! TablesListViewController
